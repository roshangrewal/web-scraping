...
# db of city
city_set = ['Bengaluru', 'New+Delhi']

# job roles
job_set = ['data+scientist', 'data+scientist+intern']
...
for city in city_set:
    ...
    for job_qry in job_set:
        ...
        if (file > SKIPPER):
            ...
            # dataframe
            df = pd.DataFrame(
                columns=['unique_id', 'city', 'job_qry', 'job_title', 'company_name', 'location', 'summary', 'salary',
                         'link', 'date', 'full_text'])
            ...
            for start in range(0, max_results_per_city, 10):
                ...
                # get dom
                page = requests.get(
                    'http://www.indeed.co.in/jobs?q=' + job_qry + '&l=' + str(city) + '&start=' + str(start))
                ...
                # debug add
                write_logs(
                    ('Completed =>') + '\t' + city + '\t' + job_qry + '\t' + str(cnt) + '\t' + str(start) + '\t' + str(
                        time.time() - startTime) + '\t' + ('file_' + str(file)))
            ...
            # debug add
            write_logs(('Skipped =>') + '\t' + city + '\t' + job_qry + '\t' + str(-1) + '\t' + str(-1) + '\t' + str(
                time.time() - startTime) + '\t' + ('file_' + str(file)))
